...
init();})(window.themeMenuConfig);function enableStickyToc(){const observer=new IntersectionObserver((entries)=>{entries.forEach((entry)=>{const id=entry.target.getAttribute("id");if(entry.intersectionRatio>0){if(delay==true){let element=document.querySelectorAll(".tableOfContentContainer li.active");if(element!=null){element.forEach((e)=>{e.classList.remove("active");});}
delay=false;}
let element=document.querySelector(`.tableOfContentContainer li a[href="#${id}"]`);if(element){element.parentElement.classList.add("active");}}else{if(document.querySelectorAll(".tableOfContentContainer li.active").length==1){delay=true;}else{let element=document.querySelector(`.tableOfContentContainer li a[href="#${id}"]`);if(element){element.parentElement.classList.remove("active");}}}});});var delay=false;document.querySelectorAll(".content h2[id]").forEach((section)=>{observer.observe(section);});document.querySelectorAll(".content h3[id]").forEach((section)=>{observer.observe(section);});}
